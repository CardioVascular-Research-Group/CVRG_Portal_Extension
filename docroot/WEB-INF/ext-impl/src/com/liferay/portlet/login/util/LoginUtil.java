...
public class LoginUtil {
	...
	public static PortletURL getLoginURL(HttpServletRequest request, long plid)
		...
		PortletURL portletURL = new PortletURLImpl(request, PortletKeys.LOGIN,
	...
	public static void login(HttpServletRequest request,
		...
		MainAuthenticator authenticator = null;
		// GlobusNEXUS authentication
		long companyId = PortalUtil.getCompanyId(request);
		
		if (PrefsPropsUtil.getBoolean(companyId, PropsKeys.GLOBUSNEXUS_AUTH_ENABLED, PropsValues.GLOBUSNEXUS_AUTH_ENABLED)){
			String[] arguments = {login, password};
			authenticator = new MainAuthenticator();
			if(authenticator.authenticate(arguments, AuthenticationMethod.GLOBUS_REST)){
				
				try{
					userId = UserLocalServiceUtil.getUserIdByEmailAddress(company.getCompanyId(), authenticator.getUserEmail());
				}
				catch(NoSuchUserException e){
					String[] username = {authenticator.getUserFirstname(), authenticator.getUserLastname()};
					userId = createNewUser(authenticator.getUserEmail(), username, request);
					if(userId == 0L){
						throw new AuthException();
					}
					else{
						User user = UserLocalServiceUtil.getUserById(userId);
						user.getExpandoBridge().setAttribute("Institution", authenticator.getUserInstitution());
					}
				}
				authResult = Authenticator.SUCCESS;		
				
				System.out.println("Globus authenticator successful.");
			} else {
				throw new AuthException();
			}
		...
		if (authResult == Authenticator.SUCCESS) {
			if (PropsValues.SESSION_ENABLE_PHISHING_PROTECTION) {
				...
				for (String protectedAttributeName : protectedAttributeNames) {
					Object protectedAttributeValue = session
							.getAttribute(protectedAttributeName);

					if (protectedAttributeValue == null) {
						continue;
					}

					protectedAttributes.put(protectedAttributeName,
							protectedAttributeValue);
				}
				...
				for (String protectedAttributeName : protectedAttributeNames) {
					Object protectedAttributeValue = protectedAttributes
							.get(protectedAttributeName);

					if (protectedAttributeValue == null) {
						continue;
					}

					session.setAttribute(protectedAttributeName,
							protectedAttributeValue);
				}
			}
			...
			Cookie idCookie = new Cookie(CookieKeys.ID,
			...
			if (secure) {
				Boolean httpsInitial = (Boolean) session
	...
	private static long createNewUser(String userEmail, String[] userName, HttpServletRequest request){
		...
		try {
			...
			return UserLocalServiceUtil.getUserIdByEmailAddress(companyId, userEmail);
	...
	public static void sendPassword(ActionRequest actionRequest,
			String fromName, String fromAddress, String toAddress,
		...
		UserLocalServiceUtil.sendPassword(company.getCompanyId(), toAddress,
				remoteAddr, remoteHost, userAgent, fromName, fromAddress,
