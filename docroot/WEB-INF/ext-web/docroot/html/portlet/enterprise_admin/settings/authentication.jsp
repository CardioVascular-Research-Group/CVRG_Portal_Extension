...
//Shibboleth handling code Chris Jurado 3 August 2011 Johns Hopkins University Institute for Computational Medicine
...
//CILogon handling code Chris Jurado 29 November 2011 Johns Hopkins University Institute for Computational Medicine
boolean cilogonAuthEnabled = ParamUtil.getBoolean(request, "settings--" + PropsKeys.CILOGON_AUTH_ENABLED + "--", PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.CILOGON_AUTH_ENABLED, PropsValues.CILOGON_AUTH_ENABLED));
String cilogonUserValue = ParamUtil.getString(request, "settings--" + PropsKeys.CILOGON_USER_VALUE + "--", PrefsPropsUtil.getString(company.getCompanyId(), PropsKeys.CILOGON_USER_VALUE, PropsValues.CILOGON_USER_VALUE));

//Globus NEXUS handling code Chris Jurado 6 August 2012 Johns Hopkins University Institute for Computational Medicine
boolean globusNexusAuthEnabled = ParamUtil.getBoolean(request, "settings--" + PropsKeys.GLOBUSNEXUS_AUTH_ENABLED + "--", PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.GLOBUSNEXUS_AUTH_ENABLED, PropsValues.GLOBUSNEXUS_AUTH_ENABLED));
...
<liferay-ui:tabs
	names="general,ldap,cas,facebook,ntlm,open-id,open-sso,siteminder,Shibboleth,CILogon,GlobusNEXUS"
	refresh="<%= false %>"
>
	<liferay-ui:section>
		<aui:fieldset>
	...
	<!-- Shibboleth field Chris Jurado 3 August 2011 Johns Hopkins University Institute for Computational Medicine-->
	<liferay-ui:section>
		<aui:fieldset>
	...
	<!-- CILogon field Chris Jurado 29 November 2011 Johns Hopkins University Institute for Computational Medicine-->
	<liferay-ui:section>
		<aui:fieldset>
			...
			<aui:input cssClass="lfr-input-text-container" label="RFC822 Name" name='<%= "settings--" + PropsKeys.CILOGON_USER_VALUE + "--" %>' type="text" value="<%= cilogonUserValue %>" />
		</aui:fieldset>
	</liferay-ui:section>
	
	
	<!-- Globus NEXUS field Chris Jurado 6 August 2012 Johns Hopkins University Institute for Computational Medicine-->
	<liferay-ui:section>
		<aui:fieldset>
			<aui:input inlineLabel="left" label="enabled" name='<%= "settings--" + PropsKeys.GLOBUSNEXUS_AUTH_ENABLED + "--" %>' type="checkbox" value="<%= globusNexusAuthEnabled %>" />
		</aui:fieldset>
	</liferay-ui:section>
</liferay-ui:tabs>
